...
// download three.js beforre starting
      ...
      >
        <Suspense> {/* suspense: lazy loading */}
          <Scene  
            ...
            boltColor={boltColor}
          /> {/* scene: the 3d model of the skateboard */}
        </Suspense>
      </Canvas>
    </div>
  );
...
function Scene({
  ...
  return (
    <group>
      <Environment files={"/hdr/warehouse-256.hdr"} /> {/* Environment: is a custom component that is used to add a 3D environment to the scene */}
      <group ref={originRef}>
        <group ref={containerRef} position={[-0.25, 0, -0.635]}>
          <group position={[0, -0.086, 0.635]}>
            <Skateboard
              ...
              boltColor={boltColor}
              constantWheelSpin
            /> {/* Skateboard: is a custom component that is used to add a 3D skateboard to the scene */}

            <Hotspot
              isVisible={!animating && showHotspot.front}
              position={[0, 0.38, 1]}
              color="#B8FC39"
            /> {/* Hotspot: is a custom component that is used to add a 3D hotspot to the scene, which means that the user can click on it to trigger an action */}

            <mesh position={[0, 0.27, 0.9]} name="front" onClick={onClick}> {/* mesh: is a custom component that is used to add a 3D mesh to the scene , which means that the user can click on it to trigger an action*/}
              <boxGeometry args={[0.6, 0.2, 0.58]} /> {/* boxGeometry: is a custom component that is used to add a 3D box to the scene */}
              <meshStandardMaterial visible={false} /> {/* meshStandardMaterial: is a custom component that is used to add a 3D material to the scene */}
            </mesh>
        ...
        </group>  {/* group: is a custom component that is used to add a 3D group to the scene */}
      </group>
      <ContactShadows opacity={0.6} position={[0, -0.08, 0]} />  {/* contactShadows: is a custom component that is used to add a 3D contact shadows to the scene */}
      <group
      ...
      >
        <Html
          ...
          zIndexRange={[1, 0]}
          occlude="blending"
        >  {/* html is a custom component that is used to add a 3D html to the scene */}
          <WavyPaths /> {/* WavyPaths: is a custom component that is used to add a 3D wavy paths to the scene */}
        </Html>
      </group>
    </group>
  );
